package com.sulake.habbo.room.object.visualization.furniture
{
   ...
   public class FurnitureParticleSystem
   {
      ...
      private var var_234:IRoomObjectSprite;
      ...
      private var var_1035:Number = 1.0;
      
      private var var_554:BitmapData;
      
      private var var_674:ColorTransform;
      
      private var var_1285:ColorTransform;
      
      private var var_1546:Matrix;
      
      private var _translationMatrix:Matrix;
      
      private var var_691:Number = 1.0;
      ...
      public function FurnitureParticleSystem(param1:AnimatedFurnitureVisualization)
      {
         ...
         this.var_674 = new ColorTransform();
         this.var_674.alphaMultiplier = 1;
         this.var_1285 = new ColorTransform();
         this.var_1546 = new Matrix();
      ...
      public function setAnimation(param1:int) : void
      {
         ...
         this.var_96 = this.var_1036[param1];
         this.var_840 = false;
         this.updateCanvas();
      }
      ...
      private function updateCanvas() : void
      {
         ...
         if(this.var_1545 >= 0)
         {
            this.var_234 = this._visualization.getSprite(this.var_1545);
            if(this.var_234 && this.var_234.asset)
            {
               if(this.var_234.width <= 1 || this.var_234.height <= 1)
               {
                  return;
               }
               if(this.var_85 == null)
               {
                  this.var_85 = this.var_234.asset.clone();
                  if(this.var_674.alphaMultiplier != 1)
                  {
                     this.var_554 = new BitmapData(this.var_85.width,this.var_85.height,true,4278190080);
                  }
               }
               this.var_2080 = -this.var_234.offsetX;
               this.var_2078 = -this.var_234.offsetY;
               this.var_234.asset = this.var_85;
            }
            if(this.var_85)
            {
            ...
            if(this.var_554)
            {
               this.var_554.fillRect(this.var_554.rect,4278190080);
            }
         }
      }
      ...
      public function getSpriteYOffset(param1:int, param2:int, param3:int) : int
      {
         ...
         {
            return this.var_96.y * this.var_1035;
         }
         return 0;
      }
      ...
      {
         if(!this.var_96 || !this.var_234)
         {
            return;
         }
         if(this.var_85 && this.var_234.asset != this.var_85)
         {
            this.var_234.asset = this.var_85;
         }
         if(this.var_840)
      ...
      public function updateAnimation() : void
      {
         ...
         var _loc2_:* = 0;
         var _loc8_:int = 0;
         if(!this.var_840 && this.var_96.hasIgnited)
         {
            this.var_840 = true;
         }
         _loc8_ = this.var_1050 * this.var_1035;
         this.var_96.update();
         if(this.var_840)
         {
            if(this.var_96.roomObjectSpriteId >= 0)
            ...
            {
               this.var_85.draw(this.var_554,this.var_1546,this.var_674,BlendMode.NORMAL,null,false);
            }
            for each(_loc11_ in this.var_96.particles)
            {
               _loc2_ = Number(_loc11_.y);
               _loc3_ = this.var_2080 + (_loc11_.x - _loc11_.z) * 10 / 10 * this.var_1035;
               _loc4_ = this.var_2078 - _loc8_ + (_loc2_ + (_loc11_.x + _loc11_.z) / 2) * 10 / 10 * this.var_1035;
               ...
               if(_loc6_)
               {
                  ...
                  if(_loc11_.fade && _loc11_.alphaMultiplier < 1)
                  {
                     ...
                     this.var_1285.alphaMultiplier = _loc11_.alphaMultiplier;
                     this.var_85.draw(_loc7_,this._translationMatrix,this.var_1285,BlendMode.NORMAL,null,false);
                  }
                  else
                  {
      ...
      public function parseData(param1:XML) : void
      {
         ...
         var _loc23_:* = null;
         var _loc24_:* = null;
         this.var_2079 = parseInt(param1.@size);
         this.var_1545 = !!param1.hasOwnProperty("@canvas_id") ? int(parseInt(param1.@canvas_id)) : -1;
         this.var_1050 = !!param1.hasOwnProperty("@offset_y") ? int(parseInt(param1.@offset_y)) : 10;
         this.var_1035 = this.var_2079 / 64;
         this.var_691 = !!param1.hasOwnProperty("@blend") ? Number(Number(param1.@blend)) : Number(1);
         this.var_691 = Math.min(this.var_691,1);
         this.var_674.alphaMultiplier = this.var_691;
         for each(_loc3_ in param1.emitter)
         {
            ...
            for each(_loc23_ in _loc3_.particles.particle)
            {
               ...
               _loc19_ = _loc23_.@is_emitter == "FurnitureAliasesMessageEvent" ? false : true;
