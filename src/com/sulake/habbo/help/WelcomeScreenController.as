package com.sulake.habbo.help
{
   ...
   public class WelcomeScreenController implements IUpdateReceiver
   {
      ...
      public function WelcomeScreenController(param1:HabboHelp, param2:IHabboWindowManager, param3:IHabboConfigurationManager)
      {
         ...
         this._config = param3;
         this.var_139.toolbar.events.addEventListener(HabboToolbarEvent.const_37,this.onToolbarClicked);
         this.var_139.toolbar.events.addEventListener(HabboToolbarEvent.const_1122,this.onToolbarResized);
         var _loc4_:Boolean = Boolean(parseInt(this._config.getKey("new.identity","0")));
         this.var_1597 = Boolean(parseInt(this._config.getKey("welcome.screen.enabled","0"))) && _loc4_;
         this.var_2715 = parseInt(this._config.getKey("welcome.screen.delay","5000"));
         this.var_379 = new Point(72,10);
         this.var_198 = new Timer(this.var_2715,1);
      ...
      public function dispose() : void
      {
         ...
         if(this.var_139)
         {
            this.var_139.toolbar.events.removeEventListener(HabboToolbarEvent.const_37,this.onToolbarClicked);
            this.var_139 = null;
         }
         ...
         {
            this.var_198.stop();
            this.var_198.removeEventListener(TimerEvent.TIMER_COMPLETE,this.onTimerComplete);
            this.var_198 = null;
         }
         this._disposed = true;
      }
      ...
      public function setCategory(param1:String, param2:String = "", param3:String = "") : void
      {
         ...
         {
            this.var_1928 = "welcome.screen.title";
         }
         if(param3.length > 0)
         {
         ...
         {
            this.var_1927 = "welcome.screen.message";
         }
      }
      
      private function onTimerComplete(param1:TimerEvent) : void
      {
         var _loc2_:XmlAsset = this.var_139.assets.getAssetByName("welcome_screen_xml") as XmlAsset;
         ...
         var _loc4_:BitmapDataAsset = this.var_139.assets.getAssetByName("welcome_screen_arrow") as BitmapDataAsset;
         ...
         _loc5_.content.setParamFlag(WindowParam.const_321,false);
         _loc5_.height -= 20;
         this._window.findChildByName("close").addEventListener(WindowMouseEvent.WINDOW_EVENT_MOUSE_CLICK,this.onMouseClick);
         this.var_139.registerUpdateReceiver(this,10);
         this.var_1597 = false;
         this.var_379.y = this.var_139.toolbar.getIconLocation(this.var_1493) - this._window.height / 2;
         this._window.y = this.var_379.y;
         this._window.findChildByName("title").caption = "${" + this.var_1928 + "}";
         this._window.findChildByName("text").caption = "${" + this.var_1927 + "}";
         this.highlightToolbarIcon(true);
      }
      
      private function highlightToolbarIcon(param1:Boolean) : void
      {
         var _loc2_:HabboToolbarSetIconEvent = new HabboToolbarSetIconEvent(HabboToolbarSetIconEvent.const_90,this.var_1493);
         _loc2_.iconState = !!param1 ? "highlight_blue" : "0";
         this.var_139.toolbar.events.dispatchEvent(_loc2_);
      }
      ...
      private function onToolbarResized(param1:HabboToolbarEvent) : void
      {
         ...
         if(this.var_139)
         {
            this.var_139.removeUpdateReceiver(this);
            this.var_139.registerUpdateReceiver(this,10);
         }
         this.var_379.y = this.var_139.toolbar.getIconLocation(this.var_1493) - this._window.height / 2;
      }
      ...
      public function update(param1:uint) : void
      {
         ...
         var _loc2_:Number = Point.distance(this._window.rectangle.topLeft,this.var_379);
         if(_loc2_ > 5)
         {
            _loc3_ = Point.interpolate(this._window.rectangle.topLeft,this.var_379,0.5);
            this._window.x = _loc3_.x;
            this._window.y = _loc3_.y;
         }
         else
         {
            this._window.x = this.var_379.x;
            this._window.y = this.var_379.y;
            this.var_139.removeUpdateReceiver(this);
         }
      }
