package com.sulake.habbo.navigator
{
   ...
   public class RoomPopupCtrl extends PopupCtrl
   {
      ...
      public function RoomPopupCtrl(param1:HabboNavigator, param2:int, param3:int)
      {
         ...
         this.var_2228 = new RoomThumbnailRenderer(param1);
         this.var_1043 = new TagRenderer(param1,hideInstantly);
      }
      ...
      override public function refreshContent(param1:IWindowContainer) : void
      {
         ...
         {
            this.var_176 = IWindowContainer(param1.findChildByName("details_container"));
         }
         this.var_176.visible = true;
         Util.hideChildren(this.var_176);
         this.refreshRoomName(this.var_176,this.var_113);
         if(this.var_113.ownerName != "" && this.var_113.ownerName != "-")
         {
         }
         this.refreshOwnerName();
         this.refreshTextWithCaption("roomctg",this.var_176,!!this.var_113.event ? this.getEventCtg(this.var_113.categoryId) : this.getRoomCtg(this.var_113.categoryId));
         this.refreshRoomDesc(this.var_176,this.var_113);
         this.refreshExtraCont();
         this.refreshInfo(this.var_176,"trading_allowed",this.var_113.allowTrading);
         this.refreshInfo(this.var_176,"doormode_doorbell",this.var_113.doorMode == 1);
         this.refreshInfo(this.var_176,"doormode_password",this.var_113.doorMode == 2);
         Util.moveChildrenToColumn(this.var_176,["roomname","roomctg_cont","roomowner_cont","roomdesc","extra_cont","doormode_doorbell","doormode_password","trading_allowed"],0,0);
         this.var_176.height = Util.getLowestPoint(this.var_176);
      }
      
      private function refreshOwnerName() : void
      {
         this.refreshTextWithCaption("roomowner",this.var_176,this.var_113.ownerName);
         var _loc1_:ITextWindow = ITextWindow(this.var_176.findChildByName("roomowner"));
         if(this.var_113.ownerName != "" && this.var_113.ownerName != "-")
         {
      ...
      {
         var _loc1_:IWindowContainer = IWindowContainer(this.var_176.findChildByName("extra_cont"));
         Util.hideChildren(_loc1_);
         this.var_1043.refreshTags(_loc1_,this.var_113.tags);
         ...
         this.var_2228.refreshThumbnail(_loc1_,this.var_113.thumbnail,false);
      ...
      private function onOwnerName(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(_loc3_ != null)
         {
            navigator.mainViewCtrl.startSearch(Tabs.const_267,Tabs.const_263,_loc3_.text);
            this.hideInstantly();
         }
      }
   }
