package com.sulake.core.utils
{
   ...
   public class LibraryLoader extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1690:uint = 1;
      
      protected static const const_1688:uint = 2;
      
      protected static const const_1691:uint = 4;
      
      protected static const const_1689:uint = 3;
      
      protected static const name_3:uint = 7;
      
      public static const const_1411:int = 5;
      
      public static const LIBRARY_LOADER_FINALIZE:String = "LIBRARY_LOADER_FINALIZE";
      
      public static const const_1144:Boolean = false;
      
      public static const const_1550:int = 2;
      
      private static var var_891:Array = new Array();
      
      private static var var_721:Array = new Array();
      ...
      protected var var_670:XML;
      
      protected var var_551:Class;
      
      protected var var_453:uint = 0;
      ...
      protected var var_1263:int;
      
      protected var var_1264:int;
      
      protected var var_2880:uint = 0;
      
      protected var var_2797:Boolean = true;
      ...
      protected static function queue(param1:LibraryLoader) : void
      {
         ...
         {
            param1.var_43.load(param1.var_232,param1._context);
         }
      }
      ...
      protected static function throttle() : void
      {
         ...
         if(const_1144)
         {
            ...
            while(_loc2_ > -1)
            {
               _loc1_ = var_721[_loc2_] as LibraryLoader;
               if(_loc1_ && (_loc1_.ready || _loc1_.disposed))
               {
                  var_721.splice(_loc2_,1);
               }
               _loc2_--;
            }
            while(var_721.length < const_1550 && false)
            {
               _loc1_ = var_891.shift() as LibraryLoader;
               if(!_loc1_.ready && !_loc1_.disposed)
               {
                  var_721.push(_loc1_);
                  _loc1_.var_43.load(_loc1_.var_232,_loc1_._context);
               }
            }
         }
      }
      ...
      {
         return !!this.var_1021 ? uint(this.var_1263 - this.var_2034) : uint(getTimer() - this.var_2034);
      }
      ...
      override public function dispose() : void
      {
         if(!disposed)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DISPOSE,this.var_389,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
            try
            {
               this.var_43.close();
            }
      ...
      {
         if(this.var_1022 && !_disposed)
         {
            this.var_1022 = false;
            if(!this.var_1021 && this.var_232)
            {
               this.var_1263 = -1;
               this.var_2034 = getTimer();
               queue(this);
            }
         }
      }
      ...
      {
         if(!this.var_1021 && !_disposed && this.var_1264 > 0)
         {
            try
            {
               this.var_43.close();
            ...
            this.addRequestCounterToUrlRequest(this.var_232,const_1411 - this.var_1264);
            if(this.var_2797)
            {
            ...
            --this.var_1264;
            this.var_453 = 0;
            this.var_43.load(this.var_232,this._context);
            return true;
         }
         return false;
      }
      ...
      protected function loadEventHandler(param1:Event) : void
      {
         ...
         switch(param1.type)
         {
            ...
            case HTTPStatusEvent.HTTP_STATUS:
               ...
               this.debug("Load event STATUS " + this.var_389 + " for file \"" + this.url + "\"");
               break;
            case Event.UNLOAD:
               this.debug("Load event UNLOAD for file \"" + this.url + "\"");
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_UNLOAD,this.var_389,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case ProgressEvent.PROGRESS:
               this.debug("Load event PROGRESS for file \"" + this.url + "\"  bytes: " + this.bytesLoaded + "/" + this.bytesTotal);
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_PROGRESS,this.var_389,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case IOErrorEvent.IO_ERROR:
               this.debug("Load event IO ERROR for file \"" + this.url + "\"");
               if(!this.handleHttpStatus(this.var_389))
               {
                  this.var_1263 = getTimer();
            ...
            case TimerEvent.TIMER:
               ...
               if(!_disposed)
               {
                  this.var_453 = LibraryLoader.name_3;
         ...
         if(this.var_453 == LibraryLoader.const_1689)
         {
         ...
         if(this.var_453 == LibraryLoader.name_3)
         {
            if(this.prepareLibrary())
            {
               ...
               this.removeEventListeners();
               throttle();
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE,this.var_389,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
      ...
      protected function prepareLibrary() : Boolean
      {
         ...
         this.var_551 = this.getDefinition(this._name) as Class;
         ...
         try
         {
            xmlClass = this.var_551.manifest as Class;
            if(xmlClass == null)
            {
               return false;
            }
         ...
         try
         {
            this.var_670 = new XML(bytes.readUTFBytes(bytes.length));
         }
         catch(e:Error)
         {
            if(!retry())
      ...
      protected function debug(param1:String) : void
      {
         ...
         if(this._debug)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DEBUG,this.var_389,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         }
      }
      ...
      protected function failure(param1:String) : void
      {
         ...
         throttle();
         dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR,this.var_389,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         dispatchEvent(new Event(LIBRARY_LOADER_FINALIZE));
      }
